...
- 支持轻量联网搜索开关
- 支持本机长记忆：本机聊天累计到一定字数后会自动总结成一份“记忆摘要”，保存在当前浏览器 `localStorage`
...
开启后，本机聊天累计到约 6000 字，会在 AI 回复完成后自动总结成一份简短“记忆摘要”。之后每新增约 3000 字，会自动更新一次。
...
注意：自动总结会额外调用一次当前选择的模型，因此会消耗一点 API 用量。为了避免总结请求太大，每次只取最近约 9000 字聊天内容，并结合已有记忆摘要进行更新。
...
需要开启“本机长记忆”，并且本机聊天累计字数足够多。第一次通常在累计约 6000 字后自动生成，之后每新增约 3000 字再自动更新。
